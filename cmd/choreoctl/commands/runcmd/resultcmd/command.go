...
func NewCmdResult(f util.Factory, streams *genericclioptions.IOStreams) *cobra.Command {
	flags := NewResultFlags()

	cmd := &cobra.Command{
		Use:   "result [flags]",
		Short: "show result of a snapshot",
...
func (r *ResultOptions) Run(ctx context.Context, args []string) error {
	...
	rsp, err := snapshotClient.Result(ctx, &snapshotclient.ResultOptions{
		Proxy: r.Factory.GetProxy(),
	})
	if err != nil {
		return err
	}
	r.handleRunResponse(rsp)

	return nil
}

func (r *ResultOptions) handleRunResponse(rsp *runnerpb.Once_RunResponse) {
	if !rsp.Success {
		// failed
