...
RUN --mount=type=cache,target=/root/.cache/go-build \
    --mount=type=ssh \
    CGO_ENABLED=0 GOOS=${TARGETOS:-linux} GOARCH=${TARGETARCH} go build -a -o choreoctl cmd/choreoctl/main.go
...
#
COPY --from=builder --chown=$USERID:$USERID /workspace/choreoctl /app/
WORKDIR /app
# from now on, run as the unprivileged user
USER $USERID
ENTRYPOINT ["/app/choreoctl"]
