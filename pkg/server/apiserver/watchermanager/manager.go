...
type WatcherManager interface {
	Add(ctx context.Context, options rest.ListOptions, callback Watcher) error // Del is handled with the isDone or callBackFn result
...
func (r *watcherManager) Add(ctx context.Context, options rest.ListOptions, callback Watcher) error {
		...
		case event := <-r.watchCh:
			...
			for _, w := range r.watchers.list() {
				...
				go func() {
					...
					obj := event.Object.DeepCopyObject().(runtime.Unstructured)

					// the callback deals with filtering
					if keepGoing := w.callback.OnChange(event.Type, obj); !keepGoing {
