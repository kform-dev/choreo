...
func (r *staticPrefixApplicator) Validate(ctx context.Context, claim *ipamv1alpha1.IPClaim) error {
...
func (r *staticPrefixApplicator) validateExists(_ context.Context, claim *ipamv1alpha1.IPClaim) (bool, error) {
...
func (r *staticPrefixApplicator) validateParents(ctx context.Context, claim *ipamv1alpha1.IPClaim) error {
	pi, err := iputil.New(*claim.Spec.Prefix)
	if err != nil {
		return err
	}
...
func (r *staticPrefixApplicator) validateParent(_ context.Context, route table.Route, claim *ipamv1alpha1.IPClaim) error {
	...
	switch prefixType {
	case ipamv1alpha1.IPPrefixType_Aggregate:
		return fmt.Errorf("parent %s/%s nesting %s/%s is not possible", route.Prefix().String(), *claim.Spec.Prefix, parentClaimSummaryType, prefixType)
	case ipamv1alpha1.IPPrefixType_Network, ipamv1alpha1.IPPrefixType_Pool:
		// we only allow range and addresses -> these dont have a claimType
		if parentClaimSummaryType == string(ipamv1alpha1.IPClaimSummaryType_Address) ||
			parentClaimSummaryType == string(ipamv1alpha1.IPClaimSummaryType_Range) {
			return fmt.Errorf("parent %s not allowed in claim of type %s", parentClaimSummaryType, prefixType)
		}
		if parentClaimPrefixType == string(ipamv1alpha1.IPPrefixType_Network) ||
			parentClaimPrefixType == string(ipamv1alpha1.IPPrefixType_Pool) {
			return fmt.Errorf("parent %s/%s nesting %s/%s is not possible", route.Prefix().String(), *claim.Spec.Prefix, parentClaimSummaryType, prefixType)
		}
	default:
...
func (r *staticPrefixApplicator) validateChildren(_ context.Context, claim *ipamv1alpha1.IPClaim) error {
	...
	for _, childRoute := range childRoutes {
		...
		switch prefixType {
		case ipamv1alpha1.IPPrefixType_Aggregate: // the claim is of type aggregate
			// we only allow prefixes -> validate aggregate type
			if childClaimSummaryType == string(ipamv1alpha1.IPClaimSummaryType_Address) ||
				childClaimSummaryType == string(ipamv1alpha1.IPClaimSummaryType_Range) {
				return fmt.Errorf("child with addressing %s not allowed in claim of type %s", childClaimSummaryType, prefixType)
			}
			if childPrefixType == string(ipamv1alpha1.IPPrefixType_Aggregate) {
				return fmt.Errorf("nesting %s is not possible", childPrefixType)
			}
		case ipamv1alpha1.IPPrefixType_Network, ipamv1alpha1.IPPrefixType_Pool:
			// we only allow range and addresses -> these dont have a claimType
			if childClaimSummaryType == string(ipamv1alpha1.IPClaimSummaryType_Prefix) {
				return fmt.Errorf("child with addressing %s not allowed in claim of type %s", childClaimSummaryType, prefixType)
			}
		default:
...
func (r *staticPrefixApplicator) Apply(ctx context.Context, claim *ipamv1alpha1.IPClaim) error {
	log := log.FromContext(ctx)
	log.Debug("apply")
	pi, err := iputil.New(*claim.Spec.Prefix)
	if err != nil {
...
func (r *staticPrefixApplicator) Delete(ctx context.Context, claim *ipamv1alpha1.IPClaim) error {
	log := log.FromContext(ctx)
	log.Info("delete")
