...
func (r *staticRangeApplicator) Validate(ctx context.Context, claim *ipamv1alpha1.IPClaim) error {
...
func (r *staticRangeApplicator) validateExists(_ context.Context, claim *ipamv1alpha1.IPClaim) (bool, error) {
...
func (r *staticRangeApplicator) validateParents(ctx context.Context, claim *ipamv1alpha1.IPClaim) error {
	ipRange, err := netipx.ParseIPRange(*claim.Spec.Range)
	if err != nil {
		return err
	}
...
func (r *staticRangeApplicator) validateParent(_ context.Context, route table.Route, _ *ipamv1alpha1.IPClaim) error {
	routeLabels := route.Labels()
	parentPrefixType := routeLabels[backend.KuidIPAMIPPrefixTypeKey]

	if parentPrefixType == string(ipamv1alpha1.IPPrefixType_Aggregate) {
		return fmt.Errorf("a range is not possible with a parent of type %s", parentPrefixType)
	}
	return nil
}

func (r *staticRangeApplicator) validateChildren(_ context.Context, claim *ipamv1alpha1.IPClaim) error {
	ipRange, err := netipx.ParseIPRange(*claim.Spec.Range)
	if err != nil {
		return err
	}
...
func (r *staticRangeApplicator) Apply(ctx context.Context, claim *ipamv1alpha1.IPClaim) error {
	ipRange, err := netipx.ParseIPRange(*claim.Spec.Range)
	if err != nil {
		return err
	}
...
func (r *staticRangeApplicator) Delete(ctx context.Context, claim *ipamv1alpha1.IPClaim) error {
