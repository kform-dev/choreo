...
type Applicator interface {
	Validate(ctx context.Context, claim *ipamv1alpha1.IPClaim) error
	Apply(ctx context.Context, claim *ipamv1alpha1.IPClaim) error
	Delete(ctx context.Context, claim *ipamv1alpha1.IPClaim) error
}
...
func (r *applicator) getRoutesByOwner(_ context.Context, claim *ipamv1alpha1.IPClaim) (map[string]table.Routes, error) {
	...
	if claimSummaryType == ipamv1alpha1.IPClaimSummaryType_Address {
		var errm error
...
func (r *applicator) applyAddressInRange(ctx context.Context, claim *ipamv1alpha1.IPClaim, pi *iputil.Prefix, rangeName string, labels map[string]string) error {
...
func (r *applicator) applyRange(_ context.Context, claim *ipamv1alpha1.IPClaim, ipRange netipx.IPRange) error {
...
func (r *applicator) updateClaimPrefixStatus(ctx context.Context, claim *ipamv1alpha1.IPClaim, pi *iputil.Prefix) {
	claim.Status.Prefix = ptr.To[string](pi.String())
	if claim.GetIPPrefixType() == ipamv1alpha1.IPPrefixType_Network {
...
func (r *applicator) updateClaimAddressStatus(ctx context.Context, claim *ipamv1alpha1.IPClaim, pi *iputil.Prefix, networkParent bool) {
	claim.Status.Address = ptr.To[string](pi.String())
	if claim.GetIPPrefixType() == ipamv1alpha1.IPPrefixType_Network || networkParent {
		defaultGateway := r.getDefaultGateway(ctx, claim, pi)
		if defaultGateway != "" {
			claim.Status.DefaultGateway = ptr.To[string](defaultGateway)
		}
...
func (r *applicator) updateClaimRangeStatus(_ context.Context, claim *ipamv1alpha1.IPClaim) {
	claim.Status.Range = claim.Spec.Range
	claim.SetConditions(condv1alpha1.Ready())
}

func (r *applicator) getDefaultGateway(ctx context.Context, claim *ipamv1alpha1.IPClaim, pi *iputil.Prefix) string {
	log := log.FromContext(ctx)
...
func getRoutesFromClaim(_ context.Context, claim *ipamv1alpha1.IPClaim, pi *iputil.Prefix, networkParent bool, parentLabels map[string]string) []table.Route {
	routes := []table.Route{}
	...
	if claim.GetIPPrefixType() == ipamv1alpha1.IPPrefixType_Network || networkParent {
		if claim.GetIPClaimSummaryType() == ipamv1alpha1.IPClaimSummaryType_Prefix {
			switch {
...
func (r *applicator) delete(ctx context.Context, claim *ipamv1alpha1.IPClaim) error {
	log := log.FromContext(ctx)
	log.Debug("delete")
...
func (r *applicator) getRoutesByLabel(ctx context.Context, claim *ipamv1alpha1.IPClaim) table.Routes {
	log := log.FromContext(ctx)
